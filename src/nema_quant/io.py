...
if __name__ == '__main__':
    FILE_PATH_EXAMPLE = Path("data/EARL_TORSO_CTstudy.2400s.DOI.EQZ.att_yes.frame10.subs05.nii")
    ...
    try:
        ...
        print(f"Rango de valores: [{np.min(image_array_3d):.3f}, {np.max(image_array_3d):.3f}]")
        ...
        hot_37 = analysis.extract_circular_mask_2d((391, 391), (211, 171), (37/2)/2.0644)
        centro_37 = (211, 171)
        plt.imshow(hot_37, cmap='Reds', alpha=0.1)

        centro = (187, 184)
        hot_28 = analysis.extract_circular_mask_2d((391, 391), centro, (28/2)/2.0644)
        plt.imshow(hot_28, cmap='binary', alpha=0.1)

        centro = (187, 212)
        hot_22 = analysis.extract_circular_mask_2d((391, 391), centro, (22/2)/2.0644)
        plt.imshow(hot_22, cmap='binary', alpha=0.1)

        centro = (211, 226)
        hot_17 = analysis.extract_circular_mask_2d((391, 391), centro, (17/2)/2.0644)
        plt.imshow(hot_17, cmap='binary', alpha=0.1)

        centro = (235, 212)
        hot_13 = analysis.extract_circular_mask_2d((391, 391), centro, (13/2)/2.0644)
        plt.imshow(hot_13, cmap='binary', alpha=0.1)

        centro = (235, 184)
        hot_10 = analysis.extract_circular_mask_2d((391, 391), centro, (10/2)/2.0644)
        plt.imshow(hot_10, cmap='binary', alpha=0.1)

        points = [
            (centro_37[0]-16, centro_37[1]-28), (centro_37[0]-33, centro_37[1]-19), (centro_37[0]-40, centro_37[1]-1),
            (centro_37[0]-35, centro_37[1]+28), (centro_37[0]-39, centro_37[1]+50), (centro_37[0]-32, centro_37[1]+69),
            (centro_37[0]-15, centro_37[1]+79), (centro_37[0]+3, centro_37[1]+76), (centro_37[0]+19, centro_37[1]+65),
            (centro_37[0]+34, centro_37[1]+51), (centro_37[0]+38, centro_37[1]+28), (centro_37[0]+25, centro_37[1]-3),
        ]
        x_vals = [p[1] for p in points]
        y_vals = [p[0] for p in points]

        plt.plot(x_vals, y_vals, 'o', color='orange', markersize=31, mew=3, label='background rois', linestyle='none')  # 'o' means circular marker
        ...
        rois: List[Dict[str, Any]] = [
            {'center': (211, 171), 'diameter_mm': 37, 'color': 'red', 'alpha': 0.18, 'label': "hot_sphere_37mm"},
            {'center': (187, 184), 'diameter_mm': 28, 'color': 'orange', 'alpha': 0.18, 'label': "hot_sphere_28mm"},
            {'center': (187, 212), 'diameter_mm': 22, 'color': 'gold', 'alpha': 0.18, 'label': "hot_sphere_22mm"},
            {'center': (211, 226), 'diameter_mm': 17, 'color': 'lime', 'alpha': 0.18, 'label': "hot_sphere_17mm"},
            {'center': (235, 213), 'diameter_mm': 13, 'color': 'cyan', 'alpha': 0.18, 'label': "hot_sphere_13mm"},
            {'center': (235, 185), 'diameter_mm': 10, 'color': 'blue', 'alpha': 0.18, 'label': "hot_sphere_10mm"}
        ]
        ...
        for roi in rois:
            ...
            circ = Circle((x, y), radius_pix, edgecolor=roi['color'], facecolor=roi['color'], alpha=roi['alpha'], lw=2, label=roi['label'])
            ax.add_patch(circ)
        ...
        for dy, dx in bg_offsets:
            ...
            bg_circle = Circle((bg_x, bg_y), bg_radius_pix, edgecolor='orange',
        ...
        lung_circle = Circle((195, 209), 15/2.0644, edgecolor='lime', facecolor='none', lw=2, linestyle='--', label="")
        ax.add_patch(lung_circle)

        # --- Leyendas y detalles ---
        handles, labels = ax.get_legend_handles_labels()
        by_label = dict(zip(labels, handles))
        ax.legend(by_label.values(), by_label.keys(), loc='lower right', fontsize=12, framealpha=0.7)
