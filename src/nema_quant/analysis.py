    ...
    if save_visualizations and viz_dir:
        ...
        if ref_radius:
            save_background_visualization(
                central_slice, centers_offset, pivot_point_yx,
                ref_radius, viz_dir, slices_indices[len(slices_indices)//2]
            )
    ...
    for name, sphere_def in phantom.rois.items():
        ...
        for dz in offsets_z:
            ...
            for offset in offsets:
                ...
                roi_mask = extract_circular_mask_2d(
                    (slice_dims_yx[0], slice_dims_yx[1]), offset_center, sphere_roi['radius_vox']
                )
        ...
        logger.info(f"Found the best average counts for {name} with offset {best_offset_zyx}: {max_mean:.2f}")
        hot_sphere_counts[name] = max_mean

        # Save visualization if requested
        if save_visualizations and viz_dir:
            save_sphere_visualization(
                current_slice, name, center_yx, sphere_roi['radius_vox'],
                roi_mask, viz_dir, central_slice_idx
            )
...
def _calculate_hot_sphere_counts_offset_xy(
    ...
    for name, sphere_def in phantom.rois.items():
        ...
        logger.info(f"Found the best average counts for {name} with offset {best_offset}: {max_mean:.2f}")
        hot_sphere_counts[name] = max_mean

        if save_visualizations and viz_dir:
            roi_mask = extract_circular_mask_2d(
                (slice_dims_yx[0], slice_dims_yx[1]), best_offset, sphere_roi['radius_vox']
            )
            save_sphere_visualization(
                central_slice, name, best_offset, sphere_roi['radius_vox'],
                roi_mask, viz_dir, central_slice_idx
            )
...
def _calculate_hot_sphere_counts(
    ...
    for name, sphere_def in phantom.rois.items():
        ...
        if save_visualizations and viz_dir:
            save_sphere_visualization(
                central_slice, name, center_yx, sphere_roi['radius_vox'],
                roi_mask, viz_dir, central_slice_idx
            )
...
def calculate_nema_metrics(
    ...
    background_stats = _calculate_background_stats(
        image_data, phantom, slices_indices, cfg.ROIS.BACKGROUND_OFFSET_YX,
        save_visualizations=save_visualizations, viz_dir=viz_dir
    )
    ...
    logger.info(f"Phantom Center found at (z,y,x) : {phantom_center_zyx}")

    lung_insert_centers = extract_canny_mask(image_data, cfg.ROIS.SPACING, int(phantom_center_zyx[0]))

    results_lung = _calculate_lung_insert_counts(image_data, lung_insert_centers, CB_37, cfg.ROIS.SPACING)

    logger.info("Lung Insert Results")
    for k, v in results_lung.items():
        logger.info(f"Slice {int(k)}: {float(v):.3f}")
...
def save_sphere_visualization(
    ...
    axes[0].imshow(image_slice, cmap='gray', vmin=0, vmax=np.percentile(image_slice, 99))
    circle = patches.Circle(
        (center_yx[1], center_yx[0]), radius_vox,
    ...
    axes[2].imshow(masked_image, cmap='gray', vmin=0, vmax=np.percentile(image_slice, 99))
...
def save_background_visualization(
    ...
    for i, (y_offset, x_offset) in enumerate(centers_offset):
        ...
        circle = patches.Circle(
            (center_x, center_y), radius_vox,
            linewidth=1.5, edgecolor='cyan', facecolor='none', alpha=0.8
        )
        ax.add_patch(circle)
        ax.text(center_x, center_y, str(i+1), ha='center', va='center',
                color='yellow', fontweight='bold', fontsize=8)

    ax.plot(pivot_point_yx[1], pivot_point_yx[0], 'r+', markersize=15, markeredgewidth=3)
    ax.text(pivot_point_yx[1] + 10, pivot_point_yx[0], 'Pivot', color='red', fontweight='bold')

    ax.set_title(f'Background ROIs - Slice {slice_idx}\n{len(centers_offset)} ROIs shown')
    ax.axis('off')
